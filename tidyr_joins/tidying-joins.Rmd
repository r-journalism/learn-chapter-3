...
We're going to learn a couple new concepts while digging through this **murders** database: **tidyr** and joins.
...
Chicago is unsurpising but I was not expecting Salinias, California.

Let's keep going.

What happens if we disaggregate the data by seeing if clearance rates are different depending on the race of the victim in those metro areas.
...
A join combines two data sets by adding the columns of one dataset alongside the columns of the other, usually some of the rows of the second data set along with some rows of the first data set.

A successful join requires something consistent between two datasets to match on: keys.
...
But skip down to Buffalo-Niagara Falls, NY in the fourth row. There were 246 Black victims and about 70 percent of those cases went unsolved. Move a couple columns over and you see that there were 35 White victims and the rate of unsolved was 30 percent. That's a pretty big disparity.
